...
int main(int argc, char **argv)
{
    ...
    if (0 == strcmp(argv[1], "average")){
    ...
    } else if (0 == strcmp(argv[1], "detect_test")){
        ...
        int ret1=fscanf(fp,"%s",Inputfilename_tmp);
        if(ret1==EOF){
        printf("error:write the file\n");}
        //for(i=0;!feof(fp);i++){
        while(!(feof(fp))){
        //if((c=fgetc(fp))==EOF){break;}
        printf("----------the %dth image start processing------------->\n",count);
        count++;
        flag++;
        ...
        strcpy(filename, Inputfilename_tmp);
        strcat(filename,"\0");

        file_tmp=(char*)malloc(sizeof(char)*256);
        ...
        strcpy(outfile, Outputfilename_tmp);
        strcat(outfile,"\0");
        printf("out:%s\n",outfile);
        int fullscreen = find_arg(argc, argv, "-fullscreen");
        test_detector("cfg/coco.data", argv[2], argv[3], filename, thresh, .5, outfile, fullscreen);
        ...
        if(file_tmp!=NULL){
        free(file_tmp);file_tmp=NULL;}
        if(outfile!=NULL){
        free(outfile);outfile=NULL;}
        int ret2=fscanf(fp,"%s",Inputfilename_tmp);
        if(ret2==EOF){
        break;}
        }
        fclose(fp);
